...
def export(request):
    ...
    if request.method == 'POST':
        ...
        queryset = nessus_scan_results_db.objects.filter(username=username, scan_id__in=value_split)
        ...
        if report_type.lower() == 'json':
            ...
            response['Content-Disposition'] = 'attachment; filename="%s.json"' % 'report'
            return response
        if report_type.lower() == 'yaml':
            response = HttpResponse(dataset.yaml, content_type='application/x-yaml')
            response['Content-Disposition'] = 'attachment; filename="%s.yaml"' % 'report'
            return response
