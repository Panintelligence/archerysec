...
def list_vuln(request):
    ...
    gitlabsast_all_vuln = gitlabsast_scan_results_db.objects.filter(scan_id=scan_id, username=username).values('message', 'name', 'vul_col', 'scan_id', 'Severity', 'vuln_status').distinct()
...
def export(request):
    ...
    if request.method == 'POST':
        ...
        queryset = gitlabsast_scan_results_db.objects.filter(username=username, scan_id__in=value_split)
        dataset = gitlabsast_resource.export(queryset)
        if report_type == 'csv':
            response = HttpResponse(dataset.csv, content_type='text/csv')
            response['Content-Disposition'] = 'attachment; filename="%s.csv"' % 'gitlabsast_results'
            return response
        if report_type == 'json':
            response = HttpResponse(dataset.json, content_type='application/json')
            response['Content-Disposition'] = 'attachment; filename="%s.json"' % 'gitlabsast_results'
            return response
        if report_type == 'yaml':
            response = HttpResponse(dataset.yaml, content_type='application/x-yaml')
            response['Content-Disposition'] = 'attachment; filename="%s.yaml"' % 'gitlabsast_results'
            return response
